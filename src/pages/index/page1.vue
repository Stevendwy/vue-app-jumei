	...
	<div class="goods">
        ...
        <li class="type-item " v-for="(item, index) in types" :key="index" >
				...
				<div class="activity">
					...
					<span class="more arrow-right">更多</span>
					<ActivityList></ActivityList>
				</div>
				<section class="mixcart-list">
					<div class="tab-title">
						<span class="title-item"
							 v-for="(item,index) in tabs" @click="tabClick(index)" :class="index===tabIndex?'tab-click':''">{{item}}</span>
					</div>
					<GoodsList></GoodsList>
				</section>
			</div>
...
<script>
	...
	export default {
		...
		created (){
			//created 是数据初始化过程 axios 会自动解析 json 格式
